...
async function checkTraineeExists(email, idNumber) {
  ...
  if (!idResults.empty) {
    return { exists: true, reason: "id_number" };
  }
...
async function registerTrainee(trainee) {
  try {
    ...
    const existsCheck = await checkTraineeExists(trainee.email, trainee.id_number);
    ...
    // Store trainee data in Firestore
    await setDoc(doc(db, "trainees", userCredential.user.uid), {
    ...
    console.log(`✅ Successfully registered: ${trainee.name} (${trainee.location})`);
...
async function registerTraineesInBatches(trainees) {
  for (let i = 0; i < trainees.length; i += CONFIG.batchSize) {
    ...
    console.log(`\nProcessing batch ${Math.floor(i / CONFIG.batchSize) + 1}/${Math.ceil(trainees.length / CONFIG.batchSize)}`);
...
function generateReport() {
  console.log('\n\n======== REGISTRATION REPORT ========');
  console.log(`Total trainees processed: ${results.successful.length + results.failed.length + results.skipped.length}`);
